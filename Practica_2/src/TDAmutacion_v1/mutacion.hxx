...
mutacion::mutacion(const std::string & str){//para crear objeto mutacion a partir de la cadena que contiene una línea completa del fichero de entrada
  ...
  //ENFERMEDAD
  	std::string idEnfermedad, nameEnfermedad, dbEnfermedad;
...
std::ostream& operator<<(std::ostream& os, const mutacion& m)
{
   ...
   //os << getVectorData(m.getGenes());
   os << m.getCommon();
   //os << getVectorData(m.getCaf());
   //os << getVectorData(m.getEnfermedades());
   //os << getVectorData(m.getClnsig());
   return os;
}
